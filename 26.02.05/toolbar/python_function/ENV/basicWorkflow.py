# core/basicWorkflow.py
import hou
import logging
from utils import read_json_file

def basicWorkflow(json_path):
    """根据 JSON 文件创建节点"""
    created_nodes = {}

    # 读取 JSON 文件
    try:
        recipe_data = read_json_file(json_path)
    ...
    def create_nodes(parent_node, children_data):
        for child_name, child_info in children_data.items():
            try:
                ...
                logging.error(f"Failed to create node {child_name}: {e}")
            ...
            if "inputs" in child_info:
                for input_info in child_info["inputs"]:
                    ...
                    if from_node_name in created_nodes and child_name in created_nodes:
                        ...
                        logging.warning(f"Input node '{from_node_name}' not found for '{child_name}'.")
    ...
    logging.info(f"Nodes imported successfully at {current_path}")